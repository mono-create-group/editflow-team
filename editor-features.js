(function(){
  ...
  const PORTAL_APP_VERSION='20260906-06';
  const feature={
  ...
  function jobCardExtended(job){
    const j={...job,...readJobDraft(job.id)},lastSubmission=lastEditorSubmissionEvent(job),deliveryDate=j.deliveryDate||j.deadline||'',overdue=editorWorkIsOverdue(j),e=safeUrl(j.evidenceUrl),action=nextEditorJobAction(j),statuses=editorAllowedStatuses(j),timeline=editorTimelineState(j),deadline=editorDeadlineLabel(j),waiting=editorWaitMessage(j),jid=esc(j.id);
