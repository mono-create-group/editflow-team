(function(){
  ...
  const PORTAL_APP_VERSION='20260828-24';
  const feature={
  ...
  function injectStyles(){
    ...
    const style=document.createElement('style');style.id='editor-features-style';style.textContent=`
      ...
      .editor-sidebar-head{padding:8px 10px 16px;border-bottom:1px solid rgba(255,255,255,.14)}.editor-sidebar-head b{display:block;font-size:17px;color:#fff;line-height:1.35}.editor-sidebar-head span{display:block;margin-top:3px;font-size:12px;color:rgba(255,255,255,.68)}.editor-sidebar-section{display:grid;gap:3px}.editor-sidebar-label{padding:12px 10px 4px;color:rgba(255,255,255,.58);font-size:11px;font-weight:800;letter-spacing:.08em}.editor-sidebar-section .editor-nav-button{width:100%;justify-content:flex-start}.editor-sidebar-footer{margin-top:auto;padding-top:10px;border-top:1px solid rgba(255,255,255,.12)}
  ...
  function navHtmlExtended(){
    ...
    const desktop=`<nav class="nav editor-nav-desktop" aria-label="編集者ワークスペース"><div class="editor-sidebar-head"><b>mono.create 編集チーム</b><span>案件と連絡をここで管理</span></div><div class="editor-sidebar-section"><div class="editor-sidebar-label">案件管理</div>${work.map(button).join('')}</div><div class="editor-sidebar-section"><div class="editor-sidebar-label">コミュニケーション</div>${communication.map(button).join('')}</div><div class="editor-sidebar-section"><div class="editor-sidebar-label">ツール</div>${tools.map(button).join('')}</div><div class="editor-sidebar-footer">${button(['guide','使い方ガイド'])}${button(['suggestion','匿名目安箱'])}</div></nav>`;
  ...
  function readDispatchSubcases(){const rows=dispatchSubcaseRows();if(!rows.length)return{error:'子案件を1件以上入力してください',items:[]};const items=[];for(const row of rows){const title=row.querySelector('.new-subcase-title')?.value.trim()||'',editorDraftDateSetter=row.querySelector('.new-subcase-editor-draft-setter')?.value==='editor'?'editor':'creator',schedule={sharedDate:$('#new-shared')?.value||'',editorDraftDate:editorDraftDateSetter==='editor'?'':(row.querySelector('.new-subcase-editor-draft')?.value||''),clientDraftDate:row.querySelector('.new-subcase-client-draft')?.value||'',thumbnailDate:row.querySelector('.new-subcase-thumbnail')?.value||'',deliveryDate:row.querySelector('.new-subcase-delivery')?.value||''},instructions=row.querySelector('.new-subcase-instructions')?.value.trim()||'',requestUrl=row.querySelector('.new-subcase-request')?.value.trim()||'',sourceUrl=row.querySelector('.new-subcase-source')?.value.trim()||'';if(!title||!schedule.deliveryDate||!instructions)return{error:'すべての子案件に、案件名・納期（予定）・依頼内容を入力してください',items:[]};if(editorDraftDateSetter==='creator'&&!schedule.editorDraftDate)return{error:`「${title}」：案件追加者が設定する場合は編集者初稿日を入力してください`,items:[]};const dateError=scheduleError(schedule);if(dateError)return{error:`「${title}」：${dateError}`,items:[]};if((requestUrl&&!safeUrl(requestUrl))||(sourceUrl&&!safeUrl(sourceUrl)))return{error:`「${title}」：URLは https:// または http:// で入力してください`,items:[]};items.push({id:row.dataset.subcaseId||id(),title,editorDraftDateSetter,schedule,instructions,requestUrl,sourceUrl})}return{error:'',items}}
