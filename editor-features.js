(function(){
  ...
  const PORTAL_APP_VERSION='20260828-08';
  const feature={
  ...
  function injectStyles(){
    ...
    const style=document.createElement('style');style.id='editor-features-style';style.textContent=`
      ...
      @media(max-width:760px){.feature-grid,.feature-grid.two,.availability-calendar,.availability-bulk-grid{grid-template-columns:1fr}.update-banner{top:65px;align-items:flex-start;flex-wrap:wrap}.update-banner .btn{width:auto}.availability-day{display:grid;grid-template-columns:70px minmax(0,1fr);gap:7px}.availability-day-head{display:block}.availability-day>.field,.availability-day>.availability-time{margin-top:0}.availability-day .field.full{grid-column:1/-1}.editor-timeline{grid-template-columns:1fr;gap:2px}.editor-timeline-step{min-height:30px;padding:6px 5px 6px 28px}.editor-timeline-step:before{left:7px;top:9px}.editor-timeline-step:not(:last-child):after{left:12px;right:auto;top:19px;width:2px;height:calc(100% + 2px)}.editor-timeline-step b,.editor-timeline-step span{display:inline;font-size:12px;line-height:1.45}.editor-timeline-step span{margin:0 0 0 6px}.job-list-tab{flex:1;min-width:0}.job-type-filter{flex:1;min-width:0}.job-submit-panel{grid-template-columns:1fr}.job-submit-panel .btn{width:100%}.dispatch-create summary{font-size:14px}}
  ...
  function dashboardExtended(){
    const active=jobs.filter(activeJob),overdue=active.filter(editorWorkIsOverdue),review=active.filter(j=>['初稿提出済み','修正稿提出済み','D確認OK','初稿完成','確認待ち','修正中','FB待ち'].includes(j.status)),payable=activeAuthorization(new Date().toISOString().slice(0,7))?.jobIds?.length||0,priority=editorJobSortByDeadline([...overdue,...active.filter(j=>j.blocker)].filter((j,i,a)=>a.findIndex(x=>x.id===j.id)===i)).slice(0,6);
    const base=`${pageHead('概要','今必要な対応を一画面で確認')}<div class="card notice" style="margin-bottom:10px"><b>初めて使う方へ</b><p class="muted">案件の受け方から請求書の提出まで、順番に確認できます。</p><button class="btn small" type="button" onclick="setView('guide')">使い方ガイドを開く</button></div><div class="grid"><div class="card metric"><span>進行中</span><b>${active.length}</b></div><div class="card metric"><span>期限超過</span><b style="color:var(--red)">${overdue.length}</b></div><div class="card metric"><span>確認・修正</span><b>${review.length}</b></div><div class="card metric"><span>今月の請求候補</span><b>${payable}</b></div></div><section class="section"><div class="section-title"><h2>優先対応</h2><span>期限と停止理由から抽出</span></div><div class="editor-job-list">${editorGroupJobs(priority).map(group=>editorGroupHtml(group,'priority')).join('')||'<div class="card empty">優先対応はありません</div>'}</div></section>`;
    const open=feature.board.filter(x=>x.status==='open').length;
  ...
  function editorWorkflow(job){
    const raw=job?.workflow&&typeof job.workflow==='object'?job.workflow:{},status=String(job?.status||''),stage=['editing','director_review','client_submission','client_review','delivered'].includes(raw.stage)?raw.stage:(status==='完了'?'delivered':['初稿提出済み','修正稿提出済み'].includes(status)?'director_review':status==='D確認OK'?'client_submission':['確認待ち','FB待ち'].includes(status)?'client_review':'editing');
    return{round:Math.max(1,Number(raw.round)||1),stage,progressEvents:Array.isArray(job?.progressEvents)?job.progressEvents:[]};
  }
  function editorWorkflowLabel(stage){return({editing:'編集作業中',director_review:'ディレクター確認中',client_submission:'クライアント提出中',client_review:'クライアント確認中',delivered:'納品完了'})[stage]||'編集作業中'}
  function editorWaitMessage(job){return({director_review:'D確認待ちです。ディレクターが確認します。',client_submission:'ディレクターがクライアントへ提出中です。',client_review:'クライアント確認中です。修正指示が届くまでお待ちください。',delivered:'納品完了です。編集者側の操作はありません。'})[editorWorkflow(job).stage]||''}
  function editorAllowedStatuses(job){
    if(editorWorkflow(job).stage!=='editing')return[String(job?.status||'')].filter(Boolean);
    const current=String(job?.status||''),allowed={受注済み:['受注済み','進行中'],進行中:['進行中','初稿提出済み'],初稿完成:['初稿完成','初稿提出済み'],修正中:['修正中','修正稿提出済み']}[current];
  ...
  function editorGroupHtml(group,kind='jobs'){
    ...
    return`<details class="card editor-case-group"><summary><span><b>${esc(group.title)}</b><small>${esc(meta||'クライアント・アカウント未設定')}</small></span><span class="editor-case-group-count">${esc(summary)}</span></summary><div class="editor-case-group-body">${group.jobs.map(jobCard).join('')}</div><div class="editor-workflow-hint">進行：${esc(editorWorkflowLabel(workflow.stage))} / ${workflow.round}回目</div></details>`;
  }
  ...
  function boardHtml(){
    ...
    const empty=`<div class="card empty"><b>現在公開中の案件は0件です</b><br><span class="muted">権限エラーではありません。管理者が編集代行案件を掲載すると、ここに表示されます。</span><div class="actions" style="justify-content:center"><button class="btn primary" type="button" onclick="setView('jobs')">担当案件を見る／派遣案件を追加</button></div></div>`;
    return`${pageHead('案件を探す','親案件を開き、子案件ごとに日程と内容を確認して受託します。')}<div class="accept-howto"><span style="font-size:20px">✅</span><div><b>日程と内容を確認してから、最後の紫ボタンで受けてください</b><span>「この案件を受ける」を押すと、担当案件へ自動で移動します。</span></div></div><div class="privacy-note"><b>表示範囲</b><span>${isExternal()?'担当ディレクターから届いた案件のみです。':'公開案件と、ご自身宛ての案件のみです。'} クライアント請求額と利益は表示しません。</span></div><section class="section"><div class="editor-job-list">${editorGroupJobs(list).map(group=>`<details class="card editor-case-group"><summary><span><b>${esc(group.title)}</b><small>${esc([group.client,group.account].filter(Boolean).join(' / ')||'クライアント・アカウント未設定')}</small></span><span class="editor-case-group-count">公開中 ${group.jobs.length}件</span></summary><div class="editor-case-group-body">${group.jobs.map(x=>{const requested=x.audience==='designated'||Array.isArray(x.eligibleUids)&&x.eligibleUids.length===1;return`<article class="card board-card"><div class="job-top"><div><div class="job-title">${esc(x.title||'案件名未設定')}</div><div class="job-meta">初稿 ${esc(x.editorDraftDate||'未設定')} / 納品 ${esc(x.deliveryDate||'未設定')}</div></div>${requested?'<span class="pill red">編集リクエスト</span>':x.urgent?'<span class="pill red">緊急</span>':'<span class="pill">募集中</span>'}</div><details><summary>日程・案件内容を確認する</summary><div class="job-body">${esc(x.summary||x.instructions||'案件内容は未設定です。')}</div>${x.instructions&&x.summary?`<div class="muted">${esc(x.instructions)}</div>`:''}</details><button class="btn primary claim-button" type="button" onclick="claimBoardJob('${esc(x.id)}')">✓ この案件を受ける</button></article>`}).join('')}</div></details>`).join('')||empty}</div></section>`;
  }
  ...
  function jobCardExtended(job){
    const j={...job,...readJobDraft(job.id)},deliveryDate=j.deliveryDate||j.deadline||'',overdue=editorWorkIsOverdue(j),r=safeUrl(j.requestUrl),s=safeUrl(j.sourceUrl),e=safeUrl(j.evidenceUrl),action=nextEditorJobAction(j),statuses=editorAllowedStatuses(j),timeline=editorTimelineState(j),deadline=editorDeadlineLabel(j),waiting=editorWaitMessage(j);
    return`<article class="card job-card editor-job-card ${overdue?'notice danger':''}"><div class="job-top"><div><span class="pill">${editorJobTypeLabel(j)}</span><div class="job-title" style="margin-top:6px">${esc(j.title||'案件名未設定')}</div><div class="job-meta">${esc(j.clientDisplay||'クライアント未設定')} / ${esc(j.accountDisplay||'アカウント未設定')}</div></div>${statusPill(j.status)}</div>${j.correctionReason?`<div class="job-urgent-note danger"><b>差戻し内容</b><br>${esc(j.correctionReason)}</div>`:''}${j.blocker?`<div class="job-urgent-note danger"><b>停止・確認が必要です</b><br>${esc(j.blocker)}</div>`:''}<div class="deadline-summary ${overdue?'overdue':''}">${esc(deadline)}${overdue?'（作業中の案件）':''}</div><div class="editor-timeline" aria-label="編集進行の4段階">${timeline.map(x=>`<div class="editor-timeline-step ${x.state}"><b>${x.state==='done'?'✓ 完了':x.state==='current'?'● 現在':x.state==='optional'?'○ 必要時':'○ 未到達'}</b><span>${esc(x.label)}</span></div>`).join('')}</div>${waiting?`<div class="job-waiting"><b>現在の状況：</b>${esc(waiting)}</div>`:editorActionHtml(j,action,e)}<details class="job-detail"><summary>案件の詳細・連絡を開く</summary>${j.instructions?`<div class="job-body">${esc(j.instructions)}</div>`:''}<div class="link-row">${r?`<a class="safe-link" target="_blank" rel="noopener" href="${esc(r)}">依頼内容</a>`:''}${s?`<a class="safe-link" target="_blank" rel="noopener" href="${esc(s)}">素材</a>`:''}${e?`<a class="safe-link" target="_blank" rel="noopener" href="${esc(e)}">証跡</a>`:''}</div><div class="form-grid" oninput="saveJobDraft('${esc(j.id)}')" onchange="saveJobDraft('${esc(j.id)}')"><div class="field"><label for="job-status-${esc(j.id)}">ステータス</label><select id="job-status-${esc(j.id)}">${statuses.map(x=>`<option ${x===j.status?'selected':''}>${esc(x)}</option>`).join('')}</select></div><div class="field"><label for="job-shared-${esc(j.id)}">受注日</label><input id="job-shared-${esc(j.id)}" type="date" value="${esc(j.sharedDate||'')}"></div><div class="field"><label for="job-editor-draft-${esc(j.id)}">編集者 初稿</label><input id="job-editor-draft-${esc(j.id)}" type="date" value="${esc(j.editorDraftDate||'')}"></div><div class="field"><label for="job-client-draft-${esc(j.id)}">クライアント提出 初稿</label><input id="job-client-draft-${esc(j.id)}" type="date" value="${esc(j.clientDraftDate||'')}"></div><div class="field"><label for="job-thumbnail-${esc(j.id)}">サムネイル納品日</label><input id="job-thumbnail-${esc(j.id)}" type="date" value="${esc(j.thumbnailDate||'')}"></div><div class="field"><label for="job-delivery-${esc(j.id)}">納品日 *</label><input id="job-delivery-${esc(j.id)}" type="date" value="${esc(deliveryDate)}"></div><div class="field"><label for="job-workdate-${esc(j.id)}">作業日</label><input id="job-workdate-${esc(j.id)}" type="date" value="${esc(j.workDate||'')}"></div><div class="field"><label for="job-start-${esc(j.id)}">開始時刻</label><input id="job-start-${esc(j.id)}" type="time" value="${esc(j.startTime||'')}"></div><div class="field full"><label for="job-progress-${esc(j.id)}">進捗メモ</label><textarea id="job-progress-${esc(j.id)}" maxlength="2000">${esc(j.progress||'')}</textarea></div><div class="field"><label for="job-evidence-${esc(j.id)}">提出証跡URL</label><input id="job-evidence-${esc(j.id)}" type="url" value="${esc(j.evidenceUrl||'')}" placeholder="https://"></div><div class="field"><label for="job-blocker-${esc(j.id)}">停止理由</label><input id="job-blocker-${esc(j.id)}" maxlength="300" value="${esc(j.blocker||'')}"></div></div><div class="actions"><button class="btn primary job-primary" type="button" onclick="saveJobProgress('${esc(j.id)}')">変更を保存</button></div>${messageBlock(j)}</details></article>`;
  }
  ...
  function jobCardExtended(job){
    ...
    const links=`<div class="link-row">${r?`<a class="safe-link" target="_blank" rel="noopener" href="${esc(r)}">依頼内容</a>`:''}${s?`<a class="safe-link" target="_blank" rel="noopener" href="${esc(s)}">素材</a>`:''}${e?`<a class="safe-link" target="_blank" rel="noopener" href="${esc(e)}">証跡</a>`:''}</div>`;
    const fields=`<div class="form-grid" oninput="saveJobDraft('${jid}')" onchange="saveJobDraft('${jid}')"><div class="field"><label for="job-status-${jid}">ステータス</label><select id="job-status-${jid}">${statuses.map(x=>`<option ${x===j.status?'selected':''}>${esc(x)}</option>`).join('')}</select></div><div class="field"><label for="job-shared-${jid}">受注日</label><input id="job-shared-${jid}" type="date" value="${esc(j.sharedDate||'')}"></div><div class="field"><label for="job-editor-draft-${jid}">編集者 初稿</label><input id="job-editor-draft-${jid}" type="date" value="${esc(j.editorDraftDate||'')}"></div><div class="field"><label for="job-client-draft-${jid}">クライアント提出 初稿</label><input id="job-client-draft-${jid}" type="date" value="${esc(j.clientDraftDate||'')}"></div><div class="field"><label for="job-thumbnail-${jid}">サムネイル納品日</label><input id="job-thumbnail-${jid}" type="date" value="${esc(j.thumbnailDate||'')}"></div><div class="field"><label for="job-delivery-${jid}">納品日 *</label><input id="job-delivery-${jid}" type="date" value="${esc(deliveryDate)}"></div><div class="field"><label for="job-workdate-${jid}">作業日</label><input id="job-workdate-${jid}" type="date" value="${esc(j.workDate||'')}"></div><div class="field"><label for="job-start-${jid}">開始時刻</label><input id="job-start-${jid}" type="time" value="${esc(j.startTime||'')}"></div><div class="field"><label for="job-end-${jid}">終了時刻</label><input id="job-end-${jid}" type="time" value="${esc(j.endTime||'')}"></div><div class="field full"><label for="job-progress-${jid}">進捗メモ</label><textarea id="job-progress-${jid}" maxlength="2000">${esc(j.progress||'')}</textarea></div><div class="field"><label for="job-evidence-${jid}">提出証跡URL</label><input id="job-evidence-${jid}" type="url" value="${esc(j.evidenceUrl||'')}" placeholder="https://"></div><div class="field"><label for="job-blocker-${jid}">停止理由</label><input id="job-blocker-${jid}" maxlength="300" value="${esc(j.blocker||'')}"></div></div>`;
  ...
  function nextEditorJobAction(job){
    ...
    const next={受注済み:['進行中','作業を開始します','作業を開始する'],進行中:['初稿提出済み','初稿を提出します','初稿を提出した'],初稿完成:['初稿提出済み','初稿を提出します','初稿を提出した'],修正中:['修正稿提出済み','修正稿を提出します','修正稿を提出した']}[String(job?.status||'')];
    return next||null;
  }
  ...
  guideHtml=()=>editorGuideBase()
    .replace('案件を受ける、日程を伝える、質問する、初稿・修正稿・納品を提出する、請求書を送る操作はすべてここで行います。','案件を受ける、日程を伝える、質問する、初稿・修正稿を提出する操作はここで行います。D確認・クライアント提出・納品完了はディレクターまたは管理者が進めます。')
    .replace('<li>DのOKが出たら「DのOKが出た」を必ず押します。</li><li>クライアントのOKが出て納品したら「クライアントOK・納品完了」を必ず押します。</li>','<li>初稿・修正稿を提出後は「D確認待ち」となります。D確認・クライアント提出・クライアント確認・納品完了はディレクターまたは管理者が更新します。</li>')
    .replace('案件で発生した節目が「✓」になり、担当編集者本人の更新履歴と証跡URLが残っています。','初稿・修正稿の提出履歴と証跡URLが残り、その後の確認・納品工程はディレクターまたは管理者が進めます。')
    .replace('各節目は管理者ではなく、担当編集者本人が更新します。修正がなかった案件は「修正稿を提出」を未記録のままD確認OKへ進めます。確認中・修正中・提出済み・D確認OKは期限超過には数えません。','編集者が更新するのは初稿・修正稿の提出だけです。D確認・クライアント提出・修正指示・納品完了はディレクターまたは管理者が更新します。確認中・修正中・提出済み・D確認OKは期限超過には数えません。');
