(function(){
  ...
  const PORTAL_APP_VERSION='20260828-11';
  const feature={
  ...
  function injectStyles(){
    ...
    const style=document.createElement('style');style.id='editor-features-style';style.textContent=`
      ...
      .editor-nav-more{position:relative}.editor-nav-more summary{list-style:none}.editor-nav-more summary::-webkit-details-marker{display:none}.editor-nav-more-menu{position:absolute;top:calc(100% + 6px);right:0;z-index:25;display:grid;gap:6px;min-width:190px;padding:8px;border:1px solid var(--border);border-radius:10px;background:var(--card);box-shadow:var(--shadow)}.editor-nav-more-menu .btn{text-align:left;min-height:44px}.editor-nav-mobile{display:none}.editor-primary-action{border:2px solid #7c3aed;background:#faf5ff}.editor-primary-action .actions{margin-top:10px}.editor-next-child{display:block;margin-top:4px;font-size:12px;color:#5b21b6;font-weight:750}.editor-case-group>summary>span:first-child{min-width:0}.editor-case-group>summary b,.editor-case-group>summary small{overflow-wrap:anywhere}
  ...
  function navHtmlExtended(){
    const items=[['dashboard','ホーム'],['jobs','担当案件'],['board','案件を探す'],['notifications','通知'],['schedule','スケジュール'],['manuals','マニュアル'],['invoices','請求書'],['settings','登録情報'],['guide','使い方ガイド'],['suggestion','匿名目安箱']];
  ...
  function dashboardExtended(){
    ...
    const nextAction=next?`<section class="card editor-primary-action"><div class="section-title"><h2>今日、次にすること</h2><span>${esc(editorDeadlineLabel(next))}</span></div><b>${esc(editorNotificationTitle(next))}</b><div class="muted">${esc(editorWaitMessage(next)||nextEditorJobAction(next)?.[1]||'案件の詳細を確認してください。')}</div><div class="actions"><button class="btn primary" type="button" onclick="openEditorJob('${esc(next.id)}')">この案件を開く</button></div></section>`:`<section class="card editor-primary-action"><div class="section-title"><h2>今日、次にすること</h2></div><b>進行中の案件はありません</b><div class="actions"><button class="btn primary" type="button" onclick="setView('board')">案件を探す</button></div></section>`;
    const base=`${pageHead('ホーム','今日必要な対応を確認')} ${nextAction}<div class="grid"><div class="card metric"><span>進行中</span><b>${active.length}</b></div><div class="card metric"><span>期限超過</span><b style="color:var(--red)">${overdue.length}</b></div><div class="card metric"><span>確認・修正</span><b>${review.length}</b></div><div class="card metric"><span>今月の請求候補</span><b>${payable}</b></div></div><section class="section"><div class="section-title"><h2>優先対応</h2><span>期限と停止理由から抽出</span></div><div class="editor-job-list">${editorGroupJobs(priority).map(group=>editorGroupHtml(group,'priority')).join('')||'<div class="card empty">優先対応はありません</div>'}</div></section>`;
    const open=feature.board.filter(x=>x.status==='open').length;
    const availability=feature.schedules.find(x=>x.id===portalUid());
    const intro=`<details class="section"><summary class="muted" style="cursor:pointer">初めての方・表示範囲を確認</summary><div class="feature-grid two" style="margin-top:8px"><div class="card notice"><b>${isExternal()?'外部編集者':'mono.create 直接契約編集者'}</b><div class="muted">${isExternal()?'担当ディレクターの案件・支払いのみ表示します。':'クライアント請求額・利益・他の編集者の報酬は表示しません。'}</div><button class="btn small" type="button" onclick="setView('guide')">使い方ガイド</button></div><div class="card"><div class="muted">受けられる編集代行案件</div><b style="font-size:24px">${open}</b><div class="muted">${availability?.available?`稼働可 ${esc(availability.fromDate||'')} 〜 ${esc(availability.toDate||'')}`:'スケジュール未登録'}</div></div></div></details>`;
    return base+intro;
  }
  ...
  function jobCardExtended(job){
    ...
    const statusControl=waiting?`<div class="field"><label>ステータス</label><div class="editor-readonly-status" aria-label="ステータス ${esc(j.status||'未設定')}">${esc(j.status||'未設定')}（ディレクター・管理者が更新）</div><input id="job-status-${jid}" type="hidden" value="${esc(j.status||'')}"></div>`:`<div class="field"><label for="job-status-${jid}">ステータス</label><select id="job-status-${jid}">${statuses.map(x=>`<option ${x===j.status?'selected':''}>${esc(x)}</option>`).join('')}</select></div>`;
