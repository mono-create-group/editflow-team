(function(){
  ...
  const PORTAL_APP_VERSION='20260828-29';
  const feature={
  ...
  async function createDispatchJob(){
    ...
    try{
      ...
      subcases.items.forEach(subcase=>{
        const data={recordType:'editor_portal_job',businessType:'dispatch',title:subcase.title,caseName:parentCaseName,parentCaseId,parentCaseName,clientId,sourceClientId:client.sourceClientId||client.id,clientDisplay:client.name,accountId,accountDisplay:accountItem.name,deadline:subcase.schedule.deliveryDate,...subcase.schedule,editorDraftDateSetter:subcase.editorDraftDateSetter,editorPayAmount:subcase.editorPayAmount,requestUrl:subcase.requestUrl,sourceUrl:subcase.sourceUrl,instructions:subcase.instructions,urgent,status:'受注済み',workflow:{round:1,stage:'editing'},progressEvents:[],progress:'',evidenceUrl:'',blocker:'',workDate:'',startTime:'',endTime:'',submittedByUid:user.uid,editorUid:user.uid,editorEmail:user.email||'',editorName:editorDisplayName(),directorUid:assignedDirectorUid(),source:'direct_client',createdAt:at,updatedAt:at,history:[{at,type:'created',by:user.uid,status:'受注済み'}]},ref=root.doc(subcase.id);
