(function(){
  ...
  const PORTAL_APP_VERSION='20260913-02';
  const feature={
  ...
  function editorTimelineState(job){
    const workflow=editorWorkflow(job),order=['editing','director_review','client_submission','client_review','delivered'],current=Math.max(0,order.indexOf(workflow.stage)),cancelled=String(job?.status||'')==='キャンセル';
    const editingLabel=workflow.round>1?`第${workflow.round}回 修正中`:'編集者進行中';
    return order.map((key,index)=>({key,label:[editingLabel,'初稿・修正稿提出済み','D確認OK','先方確認中','完了'][index],state:cancelled?'optional':index<current?'done':index===current?'current':'todo'}));
  }
  ...
  function editorWorkflow(job){
    const raw=job?.workflow&&typeof job.workflow==='object'?job.workflow:{},status=String(job?.status||''),stage=['editing','director_review','client_submission','client_review','delivered'].includes(raw.stage)?raw.stage:(status==='完了'?'delivered':['初稿提出済み','修正稿提出済み','FB待ち'].includes(status)?'director_review':status==='D確認OK'?'client_submission':['先方確認中','確認待ち'].includes(status)?'client_review':'editing');
  ...
  function editorAllowedStatuses(job){
    ...
    const current=String(job?.status||''),legacyNoWorkflow=!(job?.workflow&&typeof job.workflow==='object'),allowed={アサイン済み:['アサイン済み','編集者進行中'],未着手:['未着手','編集者進行中'],受注済み:['受注済み','編集者進行中'],進行中:['進行中','編集者進行中'],編集者進行中:['編集者進行中','初稿提出済み'],初稿完成:legacyNoWorkflow?['初稿完成','初稿提出済み']:['初稿完成'],修正中:['修正中','修正稿提出済み']}[current];
  ...
  function editorGroupSummary(group){
    const statuses=group.jobs.map(x=>String(x.status||'')),review=statuses.filter(x=>['初稿提出済み','修正稿提出済み','D確認OK','先方確認中','確認待ち','初稿完成','修正中','FB待ち'].includes(x)).length,overdue=group.jobs.filter(editorWorkIsOverdue).length,parts=[`子案件 ${group.jobs.length}件`];
  ...
  // 直近の初稿/修正稿提出イベント。プロマネリンク・Framerリンクはルール上トップレベルに持たず、提出イベントと履歴に記録する。
  // 作業分担（社内でカット・テロップ挿入まで完了し、残り工程を編集者が担当する案件）。attachments の type:'作業分担' 行で表す。
  function editorWorkSplit(job){const items=Array.isArray(job?.attachments)?job.attachments:[];return items.find(item=>String(item?.type||'')==='作業分担')||null}
  ...
  // 修正指示は案件カードを開いたその場（注意事項の直下）に本文と画像をまとめて出す。編集者がチャット欄や
  // 過去フィードバックまで降りなくても、修正内容と参考画像が最初に目に入るようにする。
  function editorRevisionNoticeHtml(job){const reason=String(job?.correctionReason||'').trim(),images=editorRevisionImagesHtml(job);if(String(job?.status||'')!=='修正中'&&!reason)return'';if(!reason&&!images)return'';return`<section class="job-urgent-note" aria-label="Dからの修正指示"><b>Dからの修正指示</b>${reason?`<br>${esc(reason)}`:''}${images}</section>`}
  window.editorRevisionImagesHtml=editorRevisionImagesHtml;
  ...
  function jobCardExtended(job){
    ...
    const fields=`<div class="form-grid" oninput="saveJobDraft('${jid}')" onchange="saveJobDraft('${jid}')">${statusControl}<div class="field"><label for="job-shared-${jid}">受注日</label><input id="job-shared-${jid}" type="date" value="${esc(j.sharedDate||'')}"></div>${draftDateControl}<div class="field"><label for="job-client-draft-${jid}">クライアント 初稿</label><input id="job-client-draft-${jid}" type="date" value="${esc(j.clientDraftDate||'')}"></div><div class="field"><label for="job-thumbnail-${jid}">サムネイル納品日</label><input id="job-thumbnail-${jid}" type="date" value="${esc(j.thumbnailDate||'')}"></div>${deliveryControl}<div class="field"><label for="job-workdate-${jid}">作業日</label><input id="job-workdate-${jid}" type="date" value="${esc(j.workDate||'')}"></div><div class="field"><label for="job-start-${jid}">開始時刻</label><input id="job-start-${jid}" type="time" value="${esc(j.startTime||'')}"></div><div class="field"><label for="job-end-${jid}">終了時刻</label><input id="job-end-${jid}" type="time" value="${esc(j.endTime||'')}"></div><div class="field full"><label for="job-progress-${jid}">進み具合のメモ</label><textarea id="job-progress-${jid}" maxlength="2000">${esc(j.progress||'')}</textarea></div><div class="field"><label for="job-evidence-${jid}">提出した内容のURL</label><input id="job-evidence-${jid}" type="url" value="${esc(j.evidenceUrl||'')}" placeholder="https://"></div><div class="field"><label for="job-tool-${jid}">納品ツール</label><select id="job-tool-${jid}" onchange="editorDeliveryToolChanged('${jid}')"><option value="premiere" ${tool!=='capcut'?'selected':''}>Premiere Pro（プロマネリンク必須）</option><option value="capcut" ${tool==='capcut'?'selected':''}>CapCut（プロマネ・Frame.io 不要）</option></select></div><div class="field" data-tool-only="premiere" ${tool==='capcut'?'style="display:none"':''}><label for="job-pm-${jid}">プロマネリンク（Premiere の初稿・修正稿の提出時は必須）</label><input id="job-pm-${jid}" type="url" value="${esc(j.pmUrl||lastSubmission.pmUrl||'')}" placeholder="https://"></div><div class="field" data-tool-only="premiere" ${tool==='capcut'?'style="display:none"':''}><label for="job-frameio-${jid}">Frame.ioリンク（任意）</label><input id="job-frameio-${jid}" type="url" value="${esc(j.frameioUrl||lastSubmission.frameioUrl||lastSubmission.framerUrl||'')}" placeholder="https://"></div><div class="field"><label for="job-blocker-${jid}">作業を止めている理由</label><input id="job-blocker-${jid}" maxlength="300" value="${esc(j.blocker||'')}"></div></div>`;
    return`<article id="editor-job-${jid}" data-job-id="${jid}" class="card job-card editor-job-card ${overdue?'notice danger':''}"><div class="job-top"><div><span class="pill">${editorJobTypeLabel(j)}</span><div class="job-title" style="margin-top:6px">${esc(j.title||'案件名未設定')}</div><div class="job-meta">${esc(j.clientDisplay||'クライアント未設定')} / ${esc(j.accountDisplay||'アカウント未設定')}</div></div>${statusPill(j.status)}</div>${split?`<div class="notice" style="margin-top:8px"><b>社内でカット・テロップ挿入まで完了しています。残り工程を担当してください。</b><div class="muted">${esc(split.title||'')}</div>${safeUrl(split.url||'')?`<a class="btn small" style="margin-top:6px" href="${esc(safeUrl(split.url))}" target="_blank" rel="noopener noreferrer">社内編集済みプロジェクトを開く</a>`:''}</div>`:''}<div class="editor-job-status-line" aria-label="現在の工程"><span>現在の進捗</span><b>${esc(editorWorkflowLabel(editorWorkflow(j).stage,j.status))}</b></div><div class="editor-next-owner">次の担当：<b>${esc(editorNextOwner(j))}</b></div>${j.correctionReason?`<div class="job-urgent-note danger"><b>差戻し内容</b><br>${esc(j.correctionReason)}</div>`:''}${j.blocker?`<div class="job-urgent-note danger"><b>停止・確認が必要です</b><br>${esc(j.blocker)}</div>`:''}<div class="deadline-summary ${overdue?'overdue':''}">${esc(deadline)}${overdue?'（作業中の案件）':''}</div>${editorPay}<div class="editor-timeline" aria-label="編集進行の5段階">${timeline.map(x=>`<div class="editor-timeline-step ${x.state}"><b>${x.state==='done'?'完了':x.state==='current'?'いまここ':x.state==='optional'?'必要時':'未到達'}</b><span>${esc(x.label)}</span></div>`).join('')}</div>${waiting?`<div class="job-waiting"><b>現在の状況：</b>${esc(waiting)}</div>`:editorCanCompleteDelivery(j)?editorDeliveryCompletionHtml(j,e):editorActionHtml(j,action,e)}<details class="job-detail" ${feature.openMessageJobIds.has(j.id)?'open':''} ontoggle="ensureJobMessages('${jid}',this.open)"><summary>案件の詳細・連絡を開く</summary>${editorRevisionNoticeHtml(j)}${caseCautionHtml(j)}${caseManualCardsHtml(j)}${j.instructions?`<div class="job-body">${esc(j.instructions)}</div>`:''}${links}${fields}<div class="actions"><button class="btn primary job-primary" type="button" onclick="saveJobProgress('${jid}')">変更を保存</button></div>${messageBlock(j)}</details></article>`;
  }
  ...
  function editorProgressOptionLabel(job,status){
    ...
    if(status===current)return`${videoStatusLabel(status)}（現在）`;
    return({編集者進行中:'編集を開始（編集者進行中）',初稿提出済み:'初稿提出済み',修正稿提出済み:'修正稿提出済み'})[status]||videoStatusLabel(status);
  }
  ...
  function submitEditorProgressChoice(jid){
    const job=jobs.find(item=>item.id===jid),select=$('#quick-status-'+jid),status=String(select?.value||'');
    if(!job||!status)return toast('進捗を選択してください');
    if(!editorAllowedStatuses(job).includes(status))return toast('この工程はディレクターまたは管理者が更新します');
    submitEditorJobAction(jid,status);
  }
  ...
  function nextEditorJobAction(job){
    ...
    const next={アサイン済み:['編集者進行中','編集を開始します','編集を開始する'],未着手:['編集者進行中','編集を開始します','編集を開始する'],受注済み:['編集者進行中','編集を開始します','編集を開始する'],進行中:['編集者進行中','編集を開始します','編集を開始する'],編集者進行中:['初稿提出済み','初稿を提出します','初稿を提出した'],初稿完成:['初稿提出済み','初稿を提出します','初稿を提出した'],修正中:['修正稿提出済み','修正稿を提出します','修正稿を提出した']}[String(job?.status||'')];
    return next||null;
  }
  ...
  guideHtml=()=>{
    ...
    html=html
      .replace('D確認・クライアント提出・納品完了はディレクターまたは管理者が更新します。','修正中・D確認OK・先方確認中・完了はディレクターまたはオーナーが更新します。')
      .replace('案件内容・初稿日・納品日を確認します。','案件内容・初稿日・納期（予定）を確認します。')
      .replace('案件名・納品日・指示を入力します。','親案件名と子案件名、編集者初稿、クライアント初稿、納期（予定）、指示を入力します。')
      .replace('提出後は「D確認待ち」になります。D確認・クライアント提出・クライアント確認・納品完了は、ディレクターまたは管理者が更新します。','提出後は「D確認待ち」になります。修正中・D確認OK・先方確認中・完了はディレクターまたはオーナーが更新します。')
      .replace('その後の確認・納品はディレクターまたは管理者が進めます。','その後の確認・完了はディレクターまたはオーナーが進めます。')
      .replace('編集者が更新するのは初稿・修正稿の提出だけです。D確認・クライアント提出・修正指示・納品完了は、ディレクターまたは管理者が更新します。','編集者が更新するのは、編集開始と初稿・修正稿の提出です。進行中・修正中・D確認OK・先方確認中・完了はディレクターまたはオーナーが更新します。');
