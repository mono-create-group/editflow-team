(function(){
  ...
  const PORTAL_APP_VERSION='20260829-08';
  const feature={
  ...
  async function createDispatchJob(){
    ...
    const clientId=$('#new-client-id')?.value||'',accountId=$('#new-account-id')?.value||'',client=feature.catalog.find(x=>x.id===clientId),accountItem=accountItems(clientId).find(x=>x.id===accountId),requestedParentName=$('#new-case')?.value.trim()||'',parentRequestUrl=$('#new-parent-request')?.value.trim()||'',parentSourceUrl=$('#new-parent-source')?.value.trim()||'';
    ...
    try{
      ...
      subcases.items.forEach(subcase=>{
        const requestUrl=subcase.requestUrl||parentRequestUrl,sourceUrl=subcase.sourceUrl||parentSourceUrl;
