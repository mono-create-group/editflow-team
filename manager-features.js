(function(){
  ...
  function renderBoardPricingStatus(){
    ...
    if(!client){el.textContent='クライアントとアカウントを選択すると、オーナー専用の単価マスターを確認できます。';return}
    if(!state.clientPricingReady){el.textContent='クライアント単価マスターを読み込み中です。';return}
    el.textContent=pricing?.amount===null||pricing?.amount===undefined?'このクライアント・アカウントの標準単価は未設定です。クライアント一覧で設定してください。':`オーナー専用の標準単価：¥${Number(pricing.amount).toLocaleString()}（案件へは受託後に自動入力されます）`;
  }
  function hydrateAccounts(){const uid=document.getElementById('mb-editor')?.value||'',cid=document.getElementById('mb-client')?.value||'',client=clientsForEditor(uid).find(x=>x.id===cid),el=document.getElementById('mb-account');if(!el)return;const accounts=client?.accounts||[];el.innerHTML=`<option value="">${cid?accounts.length?'アカウントを選択':'アカウント未登録':'先にクライアントを選択'}</option>`+accounts.map(x=>`<option value="${esc(x.id)}">${esc(x.name)}</option>`).join('');el.onchange=renderBoardPricingStatus;renderBoardPricingStatus()}
  ...
  function requestModeChanged(){const mode=document.getElementById('mb-mode')?.value||'public',open=document.getElementById('mb-open'),target=document.getElementById('mb-editor');if(!open||!target)return;if(mode==='request'){open.checked=false;open.disabled=true;if(target.value===DIRECT_ALL_ID)target.value=state.editors[0]?.id||''}else{open.disabled=isDirector();if(_isOwner()&&target.value===DIRECT_ALL_ID)open.checked=true}hydrateClients();hydrateBoardManualSelectors()}
  function boardAudienceChanged(){const mode=document.getElementById('mb-mode')?.value||'public',open=document.getElementById('mb-open'),target=document.getElementById('mb-editor');if(mode==='request'){if(open)open.checked=false;hydrateBoardManualSelectors();return}if(open?.checked&&target&&_isOwner()){target.value=DIRECT_ALL_ID;hydrateClients()}hydrateBoardManualSelectors()}
  async function publishBoard(){
    ...
    const parentRequestUrl=document.getElementById('mb-parent-request')?.value.trim()||'',parentSourceUrl=document.getElementById('mb-parent-source')?.value.trim()||'',parentManualIds=selectedManualIds(document.getElementById('mb-parent-manuals')),parentCaution=document.getElementById('mb-parent-caution')?.value.trim()||'';
