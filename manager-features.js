(function(){
  ...
  function ensureManagerResponsiveStyles(){
    ...
    const style=document.createElement('style');style.id='manager-responsive-styles';style.textContent=`
      ...
      .manager-rate-disclosure{min-width:0;max-width:100%;overflow:hidden}
      .manager-rate-form{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:10px 8px;align-items:end;min-width:0;max-width:100%}
      .manager-rate-field{display:grid;gap:3px;min-width:0;font-size:10px;color:var(--t2)}
      .manager-rate-field input{width:100%;min-width:0;min-height:44px}
      .manager-rate-field-date,.manager-rate-save{grid-column:1/-1}
      .manager-rate-save{width:100%;min-height:44px;justify-content:center;white-space:nowrap}
  ...
  async function publishBoard(){
    ...
    const parentRequestUrl=document.getElementById('mb-parent-request')?.value.trim()||'',parentSourceUrl=document.getElementById('mb-parent-source')?.value.trim()||'',parentManualIds=combinedManualIds(selectedManualIds(document.getElementById('mb-parent-manuals')),scopedManualIdsForCase(client,account,target,openAll)),parentCautionInput=document.getElementById('mb-parent-caution')?.value.trim()||'';
    ...
    // 作業分担: 社内でカット・テロップ挿入まで完了 → 残り工程を編集者へ。prproj のリンクを必須にし、
    // attachments の型付き項目（type:'作業分担'）と注意事項で編集者へ伝える（ルール変更なし）。
    const workSplit=internal?'full':(document.getElementById('mb-work-split')?.value||'full');
    let splitCaution='';
    if(workSplit==='inhouse_cut_telop'){
      const prproj=document.getElementById('mb-split-prproj')?.value.trim()||'',tasks=(document.getElementById('mb-split-tasks')?.value.trim()||'装飾・エフェクト・BGM/SE・色調整・書き出し').slice(0,200);
      const prprojUrl=typeof _videoSafeUrl==='function'?_videoSafeUrl(prproj):(safeUrl(prproj)?prproj:'');
      if(!prprojUrl)return toast('社内編集済みプロジェクト（prproj）のURLを https:// または http:// から入力してください','warn');
      parentAttachmentRead.items.unshift({id:safeId(),type:'作業分担',title:`社内でカット・テロップ挿入まで完了 ／ 編集者の担当：${tasks}`,url:prprojUrl});
      splitCaution=`【作業分担】カットとテロップ挿入は社内で完了済みです。カットの順序・テロップの内容は変更しないでください。編集者の担当：${tasks}`;
    }
    const parentCaution=[splitCaution,parentCautionInput].filter(Boolean).join('\n').slice(0,2000);
    ...
    try{
      ...
      subcases.items.forEach((subcase,subtaskIndex)=>{
        const requestUrl=subcase.requestUrl||parentRequestUrl,sourceUrl=subcase.sourceUrl||parentSourceUrl,attachments=[...parentAttachmentRead.items,...subcase.attachments].slice(0,20),manualIds=combinedManualIds(parentManualIds,subcase.manualIds),caution=splitCaution&&subcase.caution?[splitCaution,subcase.caution].join('\n').slice(0,2000):(subcase.caution||parentCaution);
  ...
  async function sendMessage(portalUid,jid){const requested=prompt('種類（回答・修正指示・連絡など）','連絡');if(!requested)return;const allowed=['質問','回答','初稿提出','修正指示','修正稿提出','納品','連絡'],kind=allowed.includes(requested)?requested:'連絡';const body=(prompt('メッセージを入力してください')||'').trim();if(!body)return;const url=(prompt('関連URL（なければ空欄）','')||'').trim();try{await fbDb.collection('editor_portals').doc(portalUid).collection('editor_jobs').doc(jid).collection('messages').add({body,kind,url,byUid:FB_USER.uid,byName:APP_ACCESS?.name||_myEmail(),byRole:_isOwner()?'オーナー':'ディレクター',createdAt:firebase.firestore.FieldValue.serverTimestamp()});toast('案件チャットに送信しました');try{await window._stampJobChatLastMessage?.(portalUid,jid,body,FB_USER.uid)}catch(error){console.warn('job chat stamp',error&&error.code||error)}try{window.loadPortalJobChat?.(portalUid,jid,true)}catch(_){}}catch(e){console.warn(e);toast('メッセージを送信できませんでした','err')}}
