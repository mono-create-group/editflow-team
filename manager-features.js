(function(){
  ...
  function readBoardSubcases(){
    ...
    for(const row of rows){
      const title=row.querySelector('.mb-subcase-title')?.value.trim()||'',editorDraftDate=row.querySelector('.mb-subcase-draft')?.value||'',clientDraftDate=row.querySelector('.mb-subcase-client-draft')?.value||'',deliveryDate=row.querySelector('.mb-subcase-delivery')?.value||'',instructions=row.querySelector('.mb-subcase-instructions')?.value.trim()||'';
      if(!title||!deliveryDate||!instructions)return{error:'すべての子案件に、案件名・納品期限・編集指示を入力してください',items:[]};
      if(editorDraftDate&&clientDraftDate&&clientDraftDate<editorDraftDate)return{error:`「${title}」：クライアント初稿は編集者初稿以降に設定してください`,items:[]};
      if((editorDraftDate&&deliveryDate<editorDraftDate)||(clientDraftDate&&deliveryDate<clientDraftDate))return{error:`「${title}」：納品期限は初稿日以降に設定してください`,items:[]};
      ...
      items.push({id:row.dataset.subcaseId||safeId(),title,editorDraftDate,clientDraftDate,deliveryDate,instructions,requestUrl:row.querySelector('.mb-subcase-request')?.value.trim()||'',sourceUrl:row.querySelector('.mb-subcase-source')?.value.trim()||'',attachments:attachmentRead.items});
    }
    return{error:'',items};
  }
