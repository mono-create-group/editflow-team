(function(){
  ...
  function clientRateScheduleHtml(client,account=null){
    ...
    const rowsHtml=!state.clientRatesReady?'<div style="font-size:11px;color:var(--t3)">適用日つき単価を読み込み中です。</div>':rows.length?rows.map(row=>`<div style="display:grid;grid-template-columns:92px 1fr 1fr auto;gap:6px;align-items:center;padding:6px 0;border-top:1px solid var(--border);font-size:11px"><span>${esc(row.effectiveFrom||'')}</span><span>受託 ¥${Number(row.clientUnitPrice||0).toLocaleString()}</span><span>編集者 ¥${Number(row.editorPayAmount||0).toLocaleString()}</span><button class="btn ${row.active===false?'btn-g':'btn-red'} btn-xs" onclick="managerToggleClientRate('${esc(row.id)}',${row.active===false?'true':'false'})">${row.active===false?'再有効化':'無効化'}</button><span style="grid-column:1/-1;color:var(--t3)">${row.active===false?'無効':'有効'} ・ rev.${Number(row.revision||1)} ・ ${esc(row.updatedBy||row.createdBy||'')}</span></div>`).join(''):'<div style="font-size:11px;color:var(--t3)">まだ適用日つきの単価はありません。下の入力から追加してください。</div>';
    return`<details style="margin-top:8px;padding:8px;border:1px solid var(--border);border-radius:8px;background:var(--card)"><summary style="cursor:pointer;font-size:12px"><b>単価・編集者支払いの適用履歴</b>　${esc(scopeLabel)}</summary><div style="font-size:10.5px;color:var(--t3);line-height:1.6;margin:7px 0">新しく作る案件だけに適用します。過去の案件・確定済みの金額は変更しません。アカウント別がない場合はクライアント共通を使います。</div><div style="display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:6px;align-items:end"><label style="font-size:10px">適用開始日<input id="${esc(dateId)}" type="date"></label><label style="font-size:10px">クライアント単価 *<input id="${esc(priceId)}" type="number" min="1" step="1" placeholder="例: 3500"></label><label style="font-size:10px">編集者支払い *<input id="${esc(payId)}" type="number" min="0" step="1" placeholder="例: 3000"></label><button class="btn btn-p btn-sm" onclick="managerSaveClientRate('${esc(client.id)}','${esc(accountId)}')">適用履歴を保存</button></div><div style="margin-top:7px">${rowsHtml}</div></details>`;
  }
