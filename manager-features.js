(function(){
  ...
  async function publishBoard(){const target=document.getElementById('mb-editor')?.value||'',mode=document.getElementById('mb-mode')?.value||'public',clientId=document.getElementById('mb-client')?.value||'',accountId=document.getElementById('mb-account')?.value||'',client=clientsForEditor(target).find(x=>x.id===clientId),account=client?.accounts?.find(x=>x.id===accountId),title=document.getElementById('mb-title')?.value.trim()||'',deliveryDate=document.getElementById('mb-delivery')?.value||'',instructions=document.getElementById('mb-instructions')?.value.trim()||'',openAll=target===DIRECT_ALL_ID||!!document.getElementById('mb-open')?.checked;if(mode==='request'&&(!target||target===DIRECT_ALL_ID))return toast('編集リクエストを送る編集者を1名選んでください','warn');if((!openAll&&!target)||!client||!account||!title||!deliveryDate||!instructions)return toast('公開先・クライアント・アカウント・案件名・納品日・編集指示は必須です','warn');const targetEditor=openAll?null:state.editors.find(x=>x.id===target),directorUid=openAll?'':isDirector()?FB_USER.uid:(targetEditor?.editorKind==='external'?targetEditor.directorUid||'':''),data={businessType:'edit_agency',title,caseName:document.getElementById('mb-case')?.value.trim()||'',clientId:client.sourceClientId||client.id,clientName:client.name,accountId,accountName:account.name,summary:instructions.slice(0,300),instructions,requestUrl:document.getElementById('mb-request')?.value.trim()||'',sourceUrl:document.getElementById('mb-source')?.value.trim()||'',editorDraftDate:document.getElementById('mb-draft')?.value||'',clientDraftDate:'',thumbnailDate:'',deliveryDate,urgent:false,status:'open',audience:openAll?'direct':(directorUid?'director_team':'designated'),eligibleUids:openAll?[]:[target],directorUid,createdByUid:FB_USER.uid,createdByName:_myEmail(),assignedUid:'',assignedName:'',assignedAt:null,createdAt:firebase.firestore.FieldValue.serverTimestamp(),updatedAt:firebase.firestore.FieldValue.serverTimestamp()};try{await fbDb.collection('editor_job_board').doc(safeId()).set(data);toast(mode==='request'?'編集リクエストを送りました':'案件を掲載しました')}catch(e){console.warn(e);toast(mode==='request'?'編集リクエストを送れませんでした':'案件を掲載できませんでした','err')}}
