(function(root){
  ...
  const state={db:null,user:null,access:null,getJobs:null,getPortalUids:null,unsubs:[],editorRows:[],managerRows:[],open:null,submitting:false,reviewing:new Set(),started:false};
  ...
  function renderEditorPage(){
    ...
    return`<div class="page"><div class="page-head"><h1>過去フィードバック</h1><p>修正から得た学びを、承認後に自分向けマニュアルとして残します。</p></div>${form}<section><div class="section-title"><h2>自分の記録</h2></div>${list}</section></div>`;
  }
  function renderManagerPage(){
    ...
    return`<div class="page"><div class="page-head"><h1>過去フィードバック</h1><p>承認した内容だけを、該当編集者向けのマニュアルにします。</p></div>${cards}</div>`;
  }
  function start(mode='editor'){
    ...
    stop();configure();if(!state.db||!portalUid())return false;state.started=true;
    if(mode==='manager'){
      if(!isOwner()&&!isDirector())return false;
      if(isOwner()){
        const q=state.db.collectionGroup('feedback').orderBy('createdAt','desc').limit(200);
        state.unsubs.push(q.onSnapshot(s=>{state.managerRows=s.docs.map(d=>normalize(d.id,d.data()));root.render?.()},e=>{if(!root.EditflowFirestoreQuota?.handle?.(e,'past feedback'))console.warn('feedback manager read',e?.code||e)}));
      }else{
        // Firestore rules are not filters. Directors subscribe only to concrete
        // portals already scoped by their access/directorUid relationship.
        const rowsByPortal=new Map(),publish=()=>{state.managerRows=[...rowsByPortal.values()].flat().filter(canSee);root.render?.()};
        const uids=[...new Set((typeof state.getPortalUids==='function'?state.getPortalUids():[]).map(String).filter(Boolean))];
        uids.forEach(uid=>state.unsubs.push(state.db.collection('editor_portals').doc(uid).collection('feedback').orderBy('createdAt','desc').limit(200).onSnapshot(s=>{rowsByPortal.set(uid,s.docs.map(d=>normalize(d.id,d.data())));publish()},e=>{rowsByPortal.delete(uid);publish();if(!root.EditflowFirestoreQuota?.handle?.(e,'scoped past feedback'))console.warn('scoped feedback read',uid,e?.code||e)})));
      }
    }else{
      const q=state.db.collection('editor_portals').doc(portalUid()).collection('feedback').orderBy('createdAt','desc').limit(200);
      state.unsubs.push(q.onSnapshot(s=>{state.editorRows=s.docs.map(d=>normalize(d.id,d.data()));root.render?.()},e=>{if(!root.EditflowFirestoreQuota?.handle?.(e,'my past feedback'))console.warn('feedback editor read',e?.code||e)}));
    }
    return true;
  }
  function stop(){state.unsubs.splice(0).forEach(unsub=>{try{unsub()}catch(_){}});state.started=false}
  ...
  const api={configure,openFromJob,renderEditorPage,renderManagerPage,start,stop,submit,review,saveOpenDraft,_test:{normalize,isCorrection,feedbackId,manualId,canSee}};
