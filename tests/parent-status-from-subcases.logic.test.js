...
test('a brand-new subcase starts at 未着手 so adding one does not demote the parent', () => {
  ...
  assert.match(index, /const st=portalJob\?\.status\|\|\(s\.done\?'完了':\(s\.status\|\|_jobSubDefaultStatus\(_bk\)\)\);/);
  // 折りたたみカード・読み出し・保存の既定と揃っている。
  assert.match(index, /videoStatusLabel\(portal\?\.status\|\|record\?\.status\|\|'未着手'\)/);
