...
test('a failed progress save writes a redacted failure record that never blocks the user',()=>{
  ...
  const calls=[];
  const context={DEMO:false,ADMIN_PREVIEW:false,db:{collection:name=>({add:record=>{calls.push([name,record]);return Promise.resolve()}})},user:{uid:'u1',email:'e@example.com',displayName:'みゆう'},access:{name:'みゆう'},PORTAL_APP_VERSION:'20260906-16',safeUrl:v=>/^https?:\/\//.test(v),now:()=>123,navigator:{userAgent:'UA'},firebase:{firestore:{FieldValue:{serverTimestamp:()=>'TS'}}},console:{warn:()=>{}}};
  ...
  assert.equal(record.appVersion,'20260906-16');
